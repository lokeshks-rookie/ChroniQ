...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <dialog
      ...
      className={`fixed inset-0 m-auto bg-base text-ink border border-ink/15 rounded-card p-0 shadow-2xl backdrop:bg-ink/40 backdrop:backdrop-blur-[1px] w-full ${maxWidthClasses} z-50 overflow-hidden`}
    >
      <div className="flex flex-col max-h-[90vh]">
        ...
        {/* Content */}
        <div className="p-6 overflow-y-auto space-y-4 text-sm text-ink">{children}</div>

        {/* Footer */}
        {footer && (
          <div className="flex items-center justify-end gap-3 p-4 px-6 border-t border-ink/10 bg-base/50">
            {footer}
          </div>
        )}
      </div>
