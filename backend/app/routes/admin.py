...
from app.models.common import AppointmentStatus, Channel, CreatedVia, Role, utcnow
...
async def create_department(
    ...
    hosp_id = current_user.hospital_id or dept_data.get("hospital_id", "hosp_city_01")
...
async def list_admin_doctors(current_user: User = Depends(get_current_user)):
...
async def create_doctor(
    ...
    """Create a new doctor."""
    hosp_id = current_user.hospital_id or doc_data.get("hospital_id", "hosp_city_01")
...
async def delete_doctor_leave(leave_id: str):
...
async def list_admin_staff(current_user: User = Depends(get_current_user)):
...
async def create_staff(
    ...
    hosp_id = current_user.hospital_id or staff_data.get("hospital_id", "hosp_city_01")
    ...
    user = User(
        ...
        role=Role(staff_data.get("role", "receptionist")),
        hospital_id=hosp_id,
        is_active=True,
        is_verified=True,
    )
...
async def get_audit_logs(
    ...
    logs = await AuditLog.find().sort("-timestamp").limit(limit).to_list()
